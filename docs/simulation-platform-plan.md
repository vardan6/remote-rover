...
- deterministic virtual GPS derived from the terrain scene georeference

Not implemented yet:
- browser live map widget on the main dashboard
- higher-fidelity geospatial transform model beyond the current local tangent-plane compatibility layer
...
- generated into an explicit scene manifest
...
Current `3d-env` baseline:
- the generated/expanded manifest `config/terrain_scene.v1.json` is the practical source of truth for the current terrain scene
- the legacy compact `config/terrain_scene.json` is generator input during transition
- runtime simulator and GCS code should consume the expanded manifest, not procedural placement rules
